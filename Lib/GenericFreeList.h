...
#define FREELIST_TYPE_DEFINE(TYPE, NAME, VECPREFIX)                            \
	...
	{                                                                      \
		...
		freelist->cur_free_index = 0;                                  \
                                                                               ...
                                                                               \
	/* freelist_reserve */                                                 \
	void NAME##_reserve(struct NAME *freelist, const size_t new_capacity)  \
	{                                                                      \
	...
	{                                                                      \
		size_t curFreeIndex = freelist->cur_free_index;                \
                                                                               \
		/* if there are no empty nodes (when the cur_free_index is     \
		 * equal to the vector's size) reallocate appropriately */     \
		if (curFreeIndex == VECPREFIX##Vector_size(&freelist->data)) { \
			NAME##_reserve(freelist,                               \
				       VECPREFIX##Vector_size(&freelist->data) \
					       * 1.5);                         \
		}                                                              \
		size_t nextFreeIndex =                                         \
			VECPREFIX##Vector_get(&freelist->data, curFreeIndex)   \
				.next_free_index;                              \
                                                                               \
		VECPREFIX##Vector_set(&freelist->data, curFreeIndex, val);     \
                                                                               \
		freelist->cur_free_index = nextFreeIndex;                      \
                                                                               \
		return curFreeIndex;                                           \
	}                                                                      \
                                                                               \
	...
	{                                                                      \
		TYPE *tmp = VECPREFIX##Vector_get_p(&freelist->data, index);   \
