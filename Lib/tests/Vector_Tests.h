...
// More on testing swap back / pop back
START_TEST(vector_test_5)
{
	struct intVector testvec;
	intVector_init(&testvec);
	intVector_reserve(&testvec, 3);
	...
	// creating a vector of: 100, 200, 300
	intVector_push_back(&testvec, 100);
	intVector_push_back(&testvec, 200);
	intVector_push_back(&testvec, 300);

	ck_assert_int_eq(intVector_get(&testvec, 0), 100);
	ck_assert_int_eq(intVector_get(&testvec, 1), 200);
	ck_assert_int_eq(intVector_get(&testvec, 2), 300);
	ck_assert_int_eq(intVector_size(&testvec), 3);
	ck_assert_int_eq(intVector_capacity(&testvec), 3);

	// swapping back
	intVector_swap_back(&testvec, 0);
	ck_assert_int_eq(intVector_get(&testvec, 0), 300);
	ck_assert_int_eq(intVector_get(&testvec, 1), 200);
	ck_assert_int_eq(intVector_get(&testvec, 2), 100);
	ck_assert_int_eq(intVector_size(&testvec), 3);
	ck_assert_int_eq(intVector_capacity(&testvec), 3);

	// popping back
	(void)intVector_pop_back(&testvec);
	ck_assert_int_eq(intVector_get(&testvec, 0), 300);
	ck_assert_int_eq(intVector_get(&testvec, 1), 200);
	ck_assert_int_eq(intVector_size(&testvec), 2);
	ck_assert_int_eq(intVector_capacity(&testvec), 3);
	...
	// swapping back with only one elemtn
	intVector_swap_back(&testvec, 0);
...
static Suite *vector_test_suite(void)
{
	...
	tcase_add_test(tc_core, vector_test_5);
