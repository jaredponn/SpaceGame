...
// macro for checking the size and capacity of the fields of a packed vector
#define SIZE_CAP_CHECK(_focusfield, _pkdvec, _dataprefix, _size, _capacity)    \
	ck_assert_int_eq(_dataprefix##Vector_size(&_pkdvec._focusfield),       \
			 _size);                                               \
	ck_assert_int_eq(_dataprefix##Vector_capacity(&_pkdvec._focusfield),   \
			 _capacity);

#define SPARSEVEC_SIZE_CAP_CHECK(_pkdvec, _size, _capacity)                    \
	SIZE_CAP_CHECK(sparse_vector, _pkdvec, Sizet, _size, _capacity)

#define PACKED_INDICES_SIZE_CAP_CHECK(_pkdvec, _size, _capacity)               \
	SIZE_CAP_CHECK(packed_indices, _pkdvec, Sizet, _size, _capacity)

#define PACKED_DATA_SIZE_CAP_CHECK(_pkdvec, _dataprefix, _size, _capacity)     \
	SIZE_CAP_CHECK(packed_data, _pkdvec, _dataprefix, _size, _capacity)

// macros for checking if the element at the given index is equal to
#define ELEM_IS(_focusfield, _pkdvec, _dataprefix, _index, _val)               \
	ck_assert_int_eq(                                                      \
		_dataprefix##Vector_get(&_pkdvec._focusfield, _index), _val);

#define SPARSEVEC_ELEM_IS(_pkdvec, _index, _val)                               \
	ELEM_IS(sparse_vector, _pkdvec, Sizet, _index, _val)

#define PACKED_INDICES_ELEM_IS(_pkdvec, _index, _val)                          \
	ELEM_IS(packed_indices, _pkdvec, Sizet, _index, _val)

#define INT_PACKED_DATA_ELEM_IS(_pkdvec, _index, _val)                         \
	ELEM_IS(packed_data, _pkdvec, int, _index, _val)


// additional addition and deletion tests
START_TEST(packedvector_test5)
{
	struct intPackedVector pkdvec;
	/* initilizes an empty int packedvector with 5 reserved spots*/
	intPackedVector_init(&pkdvec, 5);
	...
	// clang-format off
         ...
         */
	// clang-format on
	intPackedVector_add_at(&pkdvec, 0, &(int){100});

	SPARSEVEC_SIZE_CAP_CHECK(pkdvec, 5, 5)
	PACKED_INDICES_SIZE_CAP_CHECK(pkdvec, 1, 5)
	PACKED_DATA_SIZE_CAP_CHECK(pkdvec, int, 1, 5)

	SPARSEVEC_ELEM_IS(pkdvec, 0, 0)
	PACKED_INDICES_ELEM_IS(pkdvec, 0, 0)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 0, 100)
	...
	SPARSEVEC_SIZE_CAP_CHECK(pkdvec, 5, 5)
	PACKED_INDICES_SIZE_CAP_CHECK(pkdvec, 0, 5)
	PACKED_DATA_SIZE_CAP_CHECK(pkdvec, int, 0, 5)

	SPARSEVEC_ELEM_IS(pkdvec, 0, SIZE_MAX)
	PACKED_INDICES_ELEM_IS(pkdvec, 0, 0)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 0, 100)
	...
	// removing if it is wayy out of bounds
	intPackedVector_remove_at(&pkdvec, 10210930912349010);
	...
	// clang-format off
        /* 
         * sparse_vector: [SIZE_MAX, SIZE_MAX, 0]
         * packed_indices: [2]
         * data: [100]
         */
	// clang-format on
	intPackedVector_add_at(&pkdvec, 2, &(int){100});
	...
	SPARSEVEC_ELEM_IS(pkdvec, 0, SIZE_MAX)
	SPARSEVEC_ELEM_IS(pkdvec, 1, SIZE_MAX)
	SPARSEVEC_ELEM_IS(pkdvec, 2, 0)
	PACKED_INDICES_ELEM_IS(pkdvec, 0, 2)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 0, 100)

	// clang-format off
        /* 
         * sparse_vector: [1, SIZE_MAX, 0]
         * packed_indices: [2, 0]
         * data: [100, 700]
         */
	// clang-format on
	intPackedVector_add_at(&pkdvec, 0, &(int){700});

	SPARSEVEC_SIZE_CAP_CHECK(pkdvec, 5, 5)
	PACKED_INDICES_SIZE_CAP_CHECK(pkdvec, 2, 5)
	PACKED_DATA_SIZE_CAP_CHECK(pkdvec, int, 2, 5)

	SPARSEVEC_ELEM_IS(pkdvec, 0, 1)
	SPARSEVEC_ELEM_IS(pkdvec, 1, SIZE_MAX)
	SPARSEVEC_ELEM_IS(pkdvec, 2, 0)
	PACKED_INDICES_ELEM_IS(pkdvec, 0, 2)
	PACKED_INDICES_ELEM_IS(pkdvec, 1, 0)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 0, 100)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 1, 700)

	// clang-format off
        /* 
         * sparse_vector: [1, SIZE_MAX, 0, SIZE_MAX, 2]
         * packed_indices: [2, 0, 4]
         * data: [100, 700, 900]
         */
	// clang-format on
	intPackedVector_add_at(&pkdvec, 4, &(int){900});

	SPARSEVEC_SIZE_CAP_CHECK(pkdvec, 5, 5)
	PACKED_INDICES_SIZE_CAP_CHECK(pkdvec, 3, 5)
	PACKED_DATA_SIZE_CAP_CHECK(pkdvec, int, 3, 5)

	SPARSEVEC_ELEM_IS(pkdvec, 0, 1)
	SPARSEVEC_ELEM_IS(pkdvec, 1, SIZE_MAX)
	SPARSEVEC_ELEM_IS(pkdvec, 2, 0)
	SPARSEVEC_ELEM_IS(pkdvec, 3, SIZE_MAX)
	SPARSEVEC_ELEM_IS(pkdvec, 4, 2)
	PACKED_INDICES_ELEM_IS(pkdvec, 0, 2)
	PACKED_INDICES_ELEM_IS(pkdvec, 1, 0)
	PACKED_INDICES_ELEM_IS(pkdvec, 2, 4)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 0, 100)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 1, 700)
	INT_PACKED_DATA_ELEM_IS(pkdvec, 2, 900)
...
static Suite *packedvector_test_suite(void)
{
	...
	tcase_add_test(tc_core, packedvector_test5);
