		       ...
		       const Time dt)
{
	...
	for (size_t i = 0; i < velocityDataLength; ++i) {
		...
		tmpPosition_p = PositionManager_get_p_at(positionManager_p,
							 globalIndex);
		if (tmpPosition_p != NULL) {

			// modifying the position
			SYS_addVelocityToPosition(tmpVelocity_p, tmpPosition_p,
						  dt);
		}
	}
}
	...
	struct VelocityManager *const positionManager_p, const Time dt)
{
	...
	for (size_t i = 0; i < velocityDataLength; ++i) {
		...
		// gets the corrosponding position pointer
		globalIndex = AccelerationManager_get_index_from(
			velocityManager_p, i);

		tmpVelocity_p = VelocityManager_get_p_at(positionManager_p,
							 globalIndex);
		if (tmpVelocity_p != NULL) {

			// modifying the position
			SYS_addAccelerationToVelocity(tmpAcceleration_p,
						      tmpVelocity_p, dt);
		}

		// ISSUE: tmpVelocity_p is null
	}
}
...
// -----------------------------------------
static void
SYS_addAccelerationToVelocity(const Acceleration *const acceleration_p,
			      Velocity *const velocity_p, Time dt)
{
	// calculating the change in distance
	Velocity changeInVelocity = MVT_smul((const Velocity *)acceleration_p,
					     UTI_castTimeToSecs(dt));
